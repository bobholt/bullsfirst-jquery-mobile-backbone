define([
  ...
  var SuperView = Backbone.SuperView = Backbone.View.extend({
    ...
    // ### `renderInPlace`
    //
    // Acts as a render, but on an element already existing in the markup.
    // More of a pass-through so other methods get called, but there is no
    // templating or DOM insertion
    ...
    templatize : function() {
      if (!tplCache[this.template]) {
        ...
        // Using Handlebars
        tplCache[this.template] = Handlebars.compile(this.template);
